...
density <- read.csv(paste0(dir, "/", fls[4])) %>%
...
crabsAndstems <- read.csv(paste0(dir, "/", fls[1])) %>% left_join(density %>% filter(Year == "2020"))

# porewater data
porewater <- read.csv(paste0(dir, "/", fls[5]))

# leaf clip data
leaf <- read.csv(paste0(dir, "/", fls[3]))
...
survey_cores <- read.csv(paste0(dir, "/", fls[7])) %>%
  group_by(AreaID) %>%
...
survey_N <- read.csv(paste0(dir, "/", fls[9])) %>%
  left_join(read.csv(paste0(dir, "/", fls[10]))) %>%
  left_join(read.csv(paste0(dir, "/", fls[8]))) %>%
